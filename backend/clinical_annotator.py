...
class ClinicalAnnotator:
    ...
    def _load_pharmacogenomics_db(self) -> Dict:
        return {
            'BRCA1': {
                'drugs': ['Olaparib', 'Talazoparib', 'Rucaparib'],
                'indication': 'PARP inhibitors for BRCA-mutated cancers',
                'evidence_level': 'FDA Approved',
                'response': 'Increased sensitivity to PARP inhibitors'
            },
            'BRCA2': {
            ...
            'EGFR': {
                'drugs': ['Gefitinib', 'Erlotinib', 'Osimertinib'],
                'indication': 'EGFR-mutated non-small cell lung cancer',
                'evidence_level': 'FDA Approved',
                'response': 'Targeted therapy - improved outcomes'
            },
            'TP53': {
                'drugs': ['Clinical trial consideration'],
                'indication': 'Li-Fraumeni syndrome surveillance',
                'evidence_level': 'Clinical Guidelines',
                'response': 'Enhanced cancer screening recommended'
            }
        }
    ...
    def _load_disease_associations(self) -> Dict:
        return {
            'BRCA1': {
                ...
                'cancer_risks': {
                    'Breast Cancer': '55-85% lifetime risk',
                    'Ovarian Cancer': '15-45% lifetime risk'
                },
                'screening': 'Annual MRI/mammography starting age 25-30'
            },
            'BRCA2': {
                'diseases': ['Hereditary Breast and Ovarian Cancer Syndrome'],
                'inheritance': 'Autosomal Dominant',
                ...
                'screening': 'Annual MRI/mammography'
            },
            'TP53': {
                'diseases': ['Li-Fraumeni Syndrome'],
                'inheritance': 'Autosomal Dominant',
                'cancer_risks': {
                    'Various Cancers': '90% lifetime cancer risk'
                },
                'screening': 'Comprehensive cancer screening protocol'
            },
            'EGFR': {
                'diseases': ['Non-Small Cell Lung Cancer (somatic)'],
                'inheritance': 'Somatic mutation',
                'cancer_risks': {
                    'Lung Cancer': 'Targetable mutation'
                },
                'screening': 'Tumor testing recommended'
            }
        }
    
    def annotate_variant(self, variant: dict, ml_prediction: dict) -> dict:
    ...
    def _get_drug_recommendations(self, gene: str) -> Dict:
        if gene in self.drug_gene_db:
            return self.drug_gene_db[gene]
        return {
            'drugs': [],
            'indication': 'No specific drug interactions known',
            'evidence_level': 'N/A'
        }
    
    def _get_disease_info(self, gene: str) -> Dict:
        if gene in self.disease_associations:
            return self.disease_associations[gene]
        return {
            'diseases': ['Unknown/Not in database'],
            'inheritance': 'Unknown',
            'cancer_risks': {},
            'screening': 'Consult genetic counselor'
        }
    
    def _assess_actionability(self, variant: dict, ml_prediction: dict) -> str:
    ...
    def _generate_patient_summary(self, gene: str, ml_prediction: dict) -> str:
    ...
    def generate_clinical_report(self, annotations: List[Dict]) -> Dict:
        actionable_variants = [
...
if __name__ == "__main__":
    ...
    annotation = annotator.annotate_variant(test_variant, test_ml_pred)
